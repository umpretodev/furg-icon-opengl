...
class FurgDrawer {
    private:
        void static eraseDetails(vec3 origin, GLfloat radius) {
        ...
        void static drawCircle(vec3 center, GLfloat radius) {
            ColorUtil::setOrange();
        ...
        void static drawInternalBeams(vec3 origin, GLfloat radius) {
            ColorUtil::setRed();
        ...
        static void drawInternalSquare(vec3 origin, GLfloat radius) {
            ColorUtil::setYellow();

            GLfloat RIGHT_SIDE = origin.x + (radius * .45f);
            GLfloat LEFT_SIDE = origin.x - (radius * .45f);
            GLfloat UP_SIDE = origin.y + (radius * .90f);
    ...
    public:
        static void draw(vec3 origin, GLfloat radius) {
            FurgDrawer::drawCircle(origin, radius);
            FurgDrawer::drawInternalBeams(origin, radius);
            FurgDrawer::eraseDetails(origin, radius);
            FurgDrawer::drawInternalSquare(origin, radius);
        }
};